...
The hub automatically creates a "SHAPI Voice Assistant" agent in your ElevenLabs account on first use.
...
```
Browser → WebRTC → ElevenLabs ConvAI → Voice Assistant → SHAPI Hub → Coding Agent
```

The voice connection uses WebRTC for low-latency audio streaming. The SHAPI hub provides conversation tokens and handles authentication.
