...
describe('sessions routes', () => {
    it('A2A peer discovery can request newest-updated sessions with a bounded result', async () => {
        const sessions = [
        ...
        const engine = {
            ...
            getNextScheduledAtBySessionIds: () => new Map<string, number>(),
        ...
        expect(body.sessions.map((session) => session.id)).toEqual(['new-inactive'])
    })
    ...
    describe('POST /sessions/:id/archive (tiann/hapi#916)', () => {
        ...
        it('durably archives an inactive non-archived row', async () => {
            const calls: string[] = []
            const session = createSession({ active: false })
            const { app } = createApp(session, {
                archiveSession: async (sessionId: string) => { calls.push(sessionId) }
