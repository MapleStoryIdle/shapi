...
export function ScheduleTimePicker({ onSchedule, onClose, anchorRef, pendingSchedule }: ScheduleTimePickerProps) {
    ...
    return (
        <div
            ...
            className={
                isMobilePanel
                    ? `z-50 box-border max-h-[min(85dvh,calc(var(--app-viewport-height,100dvh)-24px))] ${isContentConstrained ? 'overflow-y-auto' : 'overflow-y-visible'} rounded-xl border border-[var(--app-border)] bg-[var(--app-bg)] shadow-lg fixed inset-x-3 bottom-[calc(var(--app-safe-area-bottom)+12px)] sm:left-1/2 sm:right-auto sm:w-80 sm:-translate-x-1/2`
                ...
                {tab === 'relative' ? (
                    ...
                    <div className="flex flex-col gap-2">
                        <input
                            ...
                            className="ios-form-control w-full px-3 py-2 text-sm"
                        />
                        {specificError ? (
