...
// Pinned to the major version SHAPI has been validated against (opencode
...
function buildPluginSource(hookUrl: string, token: string): string {
    ...
    return [
        '// Generated by SHAPI. Do not edit manually.',
        '',
...
/**
 * `rootPath` is a SHAPI-managed directory only when it lives under
 ...
 * that package is resolvable from `<configDir>`. SHAPI's per-session
...
function ensurePluginRuntime(rootPath: string): void {
    if (!isHapiManagedDir(rootPath)) {
        logger.debug(`[opencode-hook] Skipping plugin runtime materialization for non-SHAPI dir: ${rootPath}`);
        return;
    }
